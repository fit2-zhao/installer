                    ...
                    withEnv(["TOKEN=$TOKEN"]) {
                        dir('installer') {
                            sh script: '''
                                ...
                                curl -XPOST -H "Authorization:token $TOKEN" -H "Content-Type:application/octet-stream" --data-binary @metersphere-ce-online-installer-${RELEASE}.tar.gz [invalid url, do not cite]
                                # ossutil -c /opt/jenkins-home/cordys/config cp -f cordys-crm-ce-online-installer-${RELEASE}.tar.gz oss://resource-fit2cloud-com/cordys/crm/releases/download/${RELEASE}/ --update
                            '''
                        }
                    }
                }
