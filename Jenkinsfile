            ...
            steps {
                withCredentials([string(credentialsId: 'ZY-GITHUB-TOKEN', variable: 'TOKEN')]) {
                    withEnv(["TOKEN=$TOKEN"]) {
                        dir('installer') {
                            sh script: """
                                release=\$(curl -XPOST -H "Authorization:token \$TOKEN" -H "Content-Type: application/json" --data "{\\"tag_name\\": \\"\$RELEASE\\", \\"target_commitish\\": \\"\$BRANCH_NAME\\", \\"name\\": \\"\$RELEASE\\", \\"body\\": \\"\\", \\"draft\\": false, \\"prerelease\\": true}" https://api.github.com/repos/cordys-dev/cordys-crm/releases)
